...
# ============================================================================
# CENTRAL DE ENVIOS MANUAIS PELO OUTLOOK WEB
# O aplicativo apenas prepara a mensagem. O envio final ocorre no Outlook Web.
# Destinatários e senha ficam protegidos em .streamlit/secrets.toml / App secrets.
# ============================================================================
st.divider()
st.subheader("📧 Central de envios manuais")
st.caption(
    "Prepare mensagens individualizadas por CD Origem ou um consolidado gerencial. "
    "O clique final em Enviar é realizado no Outlook Web."
)


def ler_secret(nome, padrao=None):
    try:
...
def carregar_destinatarios_canal():
    try:
        secao = st.secrets.get("destinatarios_canal_vermelho", {})
        return {str(chave).strip(): str(valor).strip() for chave, valor in dict(secao).items() if str(valor).strip()}
    except Exception:
        return {}
...
def resumo_canal_email(base_recorte):
    total = count_orders(base_recorte)
    vermelho = count_orders(base_recorte[base_recorte["CV"] == "Sim"])
    sem_vermelho = max(0, total - vermelho)
    percentual = vermelho / total if total else 0
    dias_ativos = int(
        base_recorte.loc[
            (base_recorte["CV"] == "Sim") & base_recorte["Pedido"].notna(), "Dia"
        ].nunique()
    )
    media_diaria = vermelho / max(1, dias_ativos)

    ranking_transportadora_email = ranking_summary(base_recorte, "Transportadora").head(8)
    linhas_transportadoras = []
    for _, linha in ranking_transportadora_email.iterrows():
        linhas_transportadoras.append(
            f"• {linha['Transportadora']}: {br_int(linha['Com Canal Vermelho'])} de "
            f"{br_int(linha['Total Pedidos'])} pedidos ({br_pct(linha['% Canal Vermelho'])})"
        )
    ...
    return {
        ...
        "transportadoras": "\n".join(linhas_transportadoras),
    }


def corpo_canal_email(cd_origem, referencia, resumo):
    return f"""Olá,

Segue o status do Indicador de Canal Vermelho.
...
• Com Canal Vermelho: {br_int(resumo['vermelho'])}
• Sem Canal Vermelho: {br_int(resumo['sem_vermelho'])}
• Percentual de Canal Vermelho: {br_pct(resumo['percentual'])}
• Média diária de Canal Vermelho: {br_dec(resumo['media_diaria'], 2)}

RANKING POR TRANSPORTADORA
{resumo['transportadoras'] or 'Sem transportadoras para o recorte selecionado.'}

Mensagem preparada manualmente pelo Indicador de Canal Vermelho.

Atenciosamente,
Torre de Logística"""


senha_central = str(ler_secret("CENTRAL_ENVIOS_SENHA", "")).strip()
destinatarios_canal = carregar_destinatarios_canal()
email_gerente = str(ler_secret("EMAIL_GERENTE_CANAL_VERMELHO", "")).strip()

if "central_canal_autenticada" not in st.session_state:
    st.session_state["central_canal_autenticada"] = False

if not senha_central:
    st.warning(
        "Configure CENTRAL_ENVIOS_SENHA nos Secrets do aplicativo para liberar a Central de envios."
    )
elif not st.session_state["central_canal_autenticada"]:
    with st.form("form_acesso_central_canal", clear_on_submit=True):
        senha_informada = st.text_input("Senha de acesso", type="password")
        acessar = st.form_submit_button(
            "🔓 Acessar Central de envios", use_container_width=True, type="primary"
        )
    if acessar:
        if senha_informada == senha_central:
            st.session_state["central_canal_autenticada"] = True
            st.rerun()
        else:
            st.error("Senha incorreta. Verifique e tente novamente.")
else:
    topo_central, coluna_sair = st.columns([6, 1])
    with topo_central:
        st.info(
            "Envio manual: revise a prévia e conclua o envio no Outlook Web. "
            "Nenhuma mensagem é disparada somente ao abrir o indicador."
        )
    with coluna_sair:
        if st.button("🔒 Sair", key="sair_central_canal", use_container_width=True):
            st.session_state["central_canal_autenticada"] = False
            st.session_state.pop("mostrar_lote_canal", None)
            st.rerun()

    referencia_envio = period_label(selected_period)
    base_email = filtered_all[filtered_all["MesRef"] == selected_period].copy()
    cds_disponiveis = unique_sorted(base_email, "CD Origem")
    cds_configurados = [cd for cd in cds_disponiveis if str(cd) in destinatarios_canal]
    ...
    with aba_individual:
        if not destinatarios_canal:
            st.warning(
                "Nenhum destinatário foi configurado na seção "
                "[destinatarios_canal_vermelho] dos Secrets."
            )
        elif not cds_configurados:
            st.warning("Nenhum CD do recorte atual possui destinatário configurado.")
        else:
            cd_escolhido = st.selectbox(
                "CD Origem para o envio", cds_configurados, key="cd_email_individual"
            )
            destinatario = destinatarios_canal[str(cd_escolhido)]
            resumo_individual = resumo_canal_email(
                base_email[base_email["CD Origem"] == cd_escolhido].copy()
            )
            assunto_individual = (
                f"Status Canal Vermelho | {cd_escolhido} | {referencia_envio}"
            )
            corpo_individual = corpo_canal_email(
                cd_escolhido, referencia_envio, resumo_individual
            )

            st.write(f"**Destinatário:** {destinatario}")
            st.write(f"**Referência:** {referencia_envio}")
            with st.expander("Visualizar prévia do e-mail", expanded=True):
                st.write(f"**Assunto:** {assunto_individual}")
                st.text_area(
                    "Corpo da mensagem",
                    corpo_individual,
                    height=420,
                    disabled=True,
                    key="previa_canal_individual",
                )

            confirmar_individual = st.checkbox(
                "Confirmo que revisei o destinatário e o conteúdo.",
                key="confirmar_canal_individual",
            )
            if confirmar_individual:
                st.link_button(
                    "📨 Abrir mensagem no Outlook Web",
                    url_outlook_email(destinatario, assunto_individual, corpo_individual),
                    use_container_width=True,
                    type="primary",
                )
            else:
    ...
    with aba_todos:
        st.caption(
            "Prepara uma mensagem para cada CD configurado. Cada link abre uma nova "
            "mensagem no Outlook Web e exige o clique final em Enviar."
        )
        if not cds_configurados:
            st.warning("Nenhum CD do recorte atual possui destinatário configurado.")
        else:
            grade_envio = pd.DataFrame([
                {
                    "CD Origem": cd,
                    "Destinatário": destinatarios_canal[str(cd)],
                    "Status": "Pronto",
                }
                for cd in cds_configurados
            ])
            st.dataframe(grade_envio, use_container_width=True, hide_index=True)
            confirmar_todos = st.checkbox(
                f"Confirmo a preparação de {len(cds_configurados)} e-mails.",
                key="confirmar_todos_canal",
            )
            if st.button(
                ...
                key="preparar_todos_canal",
            ):
                st.session_state["mostrar_lote_canal"] = True

            if st.session_state.get("mostrar_lote_canal", False):
                for cd_lote in cds_configurados:
                    destinatario_lote = destinatarios_canal[str(cd_lote)]
                    resumo_lote = resumo_canal_email(
                        base_email[base_email["CD Origem"] == cd_lote].copy()
                    )
                    assunto_lote = (
                        f"Status Canal Vermelho | {cd_lote} | {referencia_envio}"
                    )
                    corpo_lote = corpo_canal_email(
                        cd_lote, referencia_envio, resumo_lote
                    )
                    st.link_button(
                        f"✉️ Abrir {cd_lote} | {destinatario_lote}",
                        url_outlook_email(destinatario_lote, assunto_lote, corpo_lote),
                        use_container_width=True,
                    )
    ...
    with aba_gerente:
        if not email_gerente:
            st.warning(
                "Configure EMAIL_GERENTE_CANAL_VERMELHO nos Secrets para liberar o consolidado."
            )
        elif base_email.empty:
            st.warning("Não existem dados para o recorte atual.")
        else:
            resumo_geral = resumo_canal_email(base_email)
            blocos = []
            for cd_gerente in cds_disponiveis:
                resumo_cd = resumo_canal_email(
                    base_email[base_email["CD Origem"] == cd_gerente].copy()
                )
                blocos.append(
                    f"{str(cd_gerente).upper()}\n"
                    f"• Total: {br_int(resumo_cd['total'])}\n"
                    f"• Canal Vermelho: {br_int(resumo_cd['vermelho'])} "
                    f"({br_pct(resumo_cd['percentual'])})\n"
                    f"• Sem Canal Vermelho: {br_int(resumo_cd['sem_vermelho'])}"
                )

            detalhamento = "\n\n".join(blocos)
            corpo_gerente = f"""Olá,

Segue o consolidado gerencial do Indicador de Canal Vermelho.

Referência: {referencia_envio}
CDs detalhados: {len(blocos)}

RESUMO EXECUTIVO
• Total de pedidos: {br_int(resumo_geral['total'])}
• Com Canal Vermelho: {br_int(resumo_geral['vermelho'])}
• Sem Canal Vermelho: {br_int(resumo_geral['sem_vermelho'])}
• Percentual de Canal Vermelho: {br_pct(resumo_geral['percentual'])}
• Média diária de Canal Vermelho: {br_dec(resumo_geral['media_diaria'], 2)}

DETALHAMENTO POR CD ORIGEM
{detalhamento}
...
Torre de Logística"""
            assunto_gerente = (
                f"Consolidado Gerencial | Canal Vermelho | {referencia_envio}"
            )

            st.write(f"**Gerente:** {email_gerente}")
            st.write(f"**CDs no consolidado:** {len(blocos)}")
            with st.expander("Visualizar consolidado gerencial", expanded=True):
                st.write(f"**Assunto:** {assunto_gerente}")
                st.text_area(
                    "Corpo consolidado",
                    corpo_gerente,
                    height=650,
                    disabled=True,
                    key="previa_canal_gerente",
                )

            confirmar_gerente = st.checkbox(
                "Confirmo que revisei o consolidado gerencial.",
                key="confirmar_canal_gerente",
            )
            if confirmar_gerente:
                st.link_button(
                    "👔 Abrir consolidado no Outlook Web",
                    url_outlook_email(email_gerente, assunto_gerente, corpo_gerente),
                    use_container_width=True,
                    type="primary",
                )
